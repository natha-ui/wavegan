...
def decode_audio(fp, fs=None, num_channels=1, normalize=False, fast_wav=False):
    ...
    if fast_wav:
        ...
        try:
            ...
            _wav, _fs = librosa.core.load('/content/drive/MyDrive/CatMeowsDataset/Project/dataset/B_ANI01_MC_FN_SIM01_101.wav', sr=fs, mono=False)
    if _wav.ndim == 2:
...
if __name__ == "__main__":
    ...
    dataset_path = '/content/drive/MyDrive/CatMeowsDataset/Project/dataset'
    
    # List all .wav files in the dataset directory
    fps = [os.path.join(dataset_path, f) for f in os.listdir(dataset_path) if f.endswith('.wav')]
    ...
    dataset = decode_extract_and_batch(
        ...
        decode_normalize=decode_normalize,
        decode_fast_wav=decode_fast_wav,
        decode_parallel_calls=decode_parallel_calls,
        slice_randomize_offset=slice_randomize_offset,
        slice_first_only=slice_first_only,
        slice_overlap_ratio=slice_overlap_ratio,
        slice_pad_end=slice_pad_end,
        repeat=repeat,
        shuffle=shuffle,
